...
def plot_function_samples(
    ...
    kernel = config["gwi"]['prior']['kernel']
    likelihood = config["gwi"]["likelihood"]["model"]
    activation_fn = config["gwi"]["neural_net"]["activation_fn"]
    ...
    if likelihood == "Gaussian":
        ...
        plt.ylim(-3, 3)
        # with open(f"gwi_{dataset}_{kernel}_{activation_fn}.pkl", "wb") as f:
        ...
        # with open(f"gwi_{dataset}_{kernel}_{activation_fn}_{alpha_eps}.pkl", "wb") as f:
        #     pickle.dump(y_samples, f)

    plt.savefig(
        f"gwi_{kernel}_{activation_fn}_{dataset}_{alpha_eps}.pdf", 
        bbox_inches='tight'
    )
