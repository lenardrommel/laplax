...
def plot_function_samples(
    ...
    if model.config["gp"]["likelihood"]["model"] == "Gaussian":
        # Sample functions
        ...
        plt.ylim(-2, 2)
        file_name = f"gp_{kernel}_{dataset}.pkl"
        with open(file_name, "wb") as f:
            pickle.dump(f_samples, f)
    elif model.config["gp"]["likelihood"]["model"] == "Categorical":
        # Build grid
        grid_len = 25
        ...
        levels = np.linspace(0, round(y_std.max(), 5), 20)
        ...
        with open(f"GP_{kernel}_{dataset}.pkl", "wb") as f:
