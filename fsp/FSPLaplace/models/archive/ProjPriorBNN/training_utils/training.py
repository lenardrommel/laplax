...
def fit_model(
    ...
    """
    # Read configuration
    likelihood_scale = config["proj_prior_bnn"]["likelihood_scale"]
    nb_epochs = config["proj_prior_bnn"]["training"]["nb_epochs"]

    # Training data
    X_train, y_train = dataloader.get_data(data_split="train")

    # Validation data
    X_val, y_val = dataloader.get_data(data_split="val")
