...
from models.ProjPriorBNN.model_utils.mlp import MLP
from models.ProjPriorBNN.model_utils.prior import Prior
from models.ProjPriorBNN.training_utils.inference import inference
from models.ProjPriorBNN.training_utils.plot_utils import plot_function_samples
from models.ProjPriorBNN.training_utils.training import fit_model, evaluate_model
...
class ProjPriorBNN:
    ...
    def __init__(
        ...
        self.config = config
        self.stochastic_layers = config["proj_prior_bnn"]["stochastic_layers"]
    
        # Initialize model
        self.forward = hk.transform(
            self.make_forward_fn(
                config["proj_prior_bnn"]["architecture"], 
                config["proj_prior_bnn"]["activation_fn"]
            )
        )
        ...
        self.prior = Prior(
            key=key1,
            kernel=config["proj_prior_bnn"]["prior"]["kernel"], 
            kernel_params=config["proj_prior_bnn"]["prior"]["parameters"], 
        )

        # Initialize optimizer
        opt_init, self.opt_update, self.get_params = adam(
            config["proj_prior_bnn"]["training"]["lr"],
            config["proj_prior_bnn"]["training"]["b1"],
            config["proj_prior_bnn"]["training"]["b2"],
            config["proj_prior_bnn"]["training"]["eps"]
        )
    ...
    def f_predict(
        ...
        """
        # Get configuration 
        cov_type = self.config["proj_prior_bnn"]["cov_type"]
