...
def plot_function_samples(
    ...
    kernel = config["flaplace_sampling"]['prior']['kernel']
    alpha_eps = config["flaplace_sampling"]["prior"]["alpha_eps"]
    activation_fn = config["flaplace_sampling"]["neural_net"]["activation_fn"]
    ...
    plt.savefig(
        f"flin_laplace_sampling_{dataset}_{kernel}_{n_context_points}_{activation_fn}_{alpha_eps}.pdf", 
        bbox_inches='tight'
    )
