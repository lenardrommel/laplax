...
class KnowledgeDistillation(SyntheticTestFunction):
    ...
    def __init__(
        ...
        self.teacher = LeNet5(10).to(self.device)
        self.teacher.load_state_dict(torch.load("bnn_params/lenet_mnist.pt"))
    ...
    def f(self, params):
        ...
        for data, target in self.train_loader:
            student_probs = self.student(data.to(self.device))[1] + 1e-10
            teacher_probs = self.teacher(data.to(self.device))[1] + 1e-10
