...
FLAPLACE_PARAMS = {
    ...
    "cov_context_selection": {
        "distribution": "categorical",
        "values": ["latin_hypercube", "train_val_latin", "halton", "train_val_halton", "val_latin", "val_halton", "grid"]
    },
...
def hpo_era5_interpolation_experiment(
    config
):
    """
    """
    ...
    SWEEP_CONFIG['name'] = f"era5_inter_{config['data']['name']}_{config['model']['name']}"
    ...
    while not wandb_init:
        try:
            ...
            wandb.agent(sweep_id, function=lambda: era5_interpolation_experiment(config, hpo=True), count=50, project="FSPLaplace")
            wandb_init = True
...
def era5_interpolation_experiment(
    config, 
    hpo=False
):
    """
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(dataset)):
        ...
        print("Test size: ", len(test_idx), flush=True) 
        ...
        test_loader = ERA5DataLoader(key3, dataset, batch_size, shuffle=True, dataset_idx=test_idx, normalize_labels=True)
        ...
        #plot_era5(model, key, train_loader, fold, model_name, time_idx=0, save_fig=False)
        #plot_era5_time_series(model, key, dataset, train_idx, train_loader.label_stats, fold, save_fig=False)
...
def plot_era5(
    ...
    if save_fig:
        plt.savefig(f"{model_name}_era5_interpolation_{it}.png")
...
def plot_era5_time_series(
    ...
    for i, _coord in enumerate([[0,0], [0,1], [0,2], [0,3], [0,4]]):
        ...
        # Fetch data
        t2m = dataset.y[long,lat,:].reshape(-1, 1)
        print("t2m.shape", t2m.shape)

        # Fetch features
        time = (dataset.X[2] - dataset.feature_stats["t"]["mean"]) / dataset.feature_stats["t"]["std"]
        longitude = dataset.X[0][long].reshape(-1, 1)
        latitude = dataset.X[1][lat].reshape(-1, 1)        
        x = jnp.concatenate(
            (
        ...
        # Rescale the predictions
        f_samples = f_samples*label_stats["std"] + label_stats["mean"]
        ...
        # Plot ground truth
        _, _, time_idx = jnp.unravel_index(train_idx, dataset.y.shape)
        x = dataset.X[2][time_idx]
        y = dataset.y[long, lat, time_idx]
    ...
    if save_fig:
        plt.savefig(f"t2m_extrapolation_{long}_{lat}_it_{it}.pdf")
...
def init_wandb(
    ...
    while not wandb_init:
        try:
            grp_name = f"era5_inter_{config['data']['name']}_{config['model']['name']}"
            mode = "online" if config["experiment"]["name"] == "hpo_era5_interpolation" else "offline"
            wandb.init(
                project="flaplace",
                config=config, 
                group=grp_name,
